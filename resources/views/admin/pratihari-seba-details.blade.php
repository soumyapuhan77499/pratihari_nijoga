...
.form-check-input {
    ...
    cursor: pointer;
    border: 2px solid #3248c7; /* Green Border */
    border-radius: 5px;
    transition: all 0.3s ease-in-out;
}
...
.form-check-input:checked {
    background-color: #0d0d0c; /* Green Background */
    border-color: #2d11e9;
}
        ...
        <div class="col-12 mt-2">
            <div class="card">
                ...
                <ul class="nav nav-tabs flex-column flex-sm-row mt-2" role="tablist">
                    ...
                    <li class="nav-item col-12 col-sm-auto">
                        <a class="nav-link" id="social-media-tab" data-toggle="tab" href="{{ route('admin.pratihariSocialMedia') }}"
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-4 mt-3">
                                <div class="form-group">
                                    <label for="nijoga_type">🛕 Nijoga Category</label>
                                    <select class="form-control custom-select" id="nijoga_type" name="nijoga_type" required>
                            ...
                            <!-- Seba Listing (Dynamically Updated) -->
                            <div class="col-md-4 mt-3">
                                <label><i class="fas fa-tasks"></i> Available Sebas</label>
                                <div class="checkbox-list" id="seba_list">
                                    <!-- Seba checkboxes will be added dynamically -->
                                </div>
                            </div>
        
                            <!-- Beddha Listing (Dynamically Updated) -->
                            <div class="col-md-4 mt-3">
                                <label><i class="fas fa-list-ul"></i> Beddha List</label>
                                <div class="checkbox-list" id="beddha_list">
                                    <!-- Beddha checkboxes will be added dynamically -->
                                </div>
                            </div>
        
                            <div class="col-12 text-center">
                                <button type="submit" class="btn btn-lg btn-success mt-3 w-50 custom-gradient-btn">
...
<script>
   document.getElementById('nijoga_type').addEventListener('change', function () {
    ...
    sebaList.innerHTML = '';
    document.getElementById('beddha_list').innerHTML = ''; // Clear Beddha list on Nijoga change
            ...
            .then(data => {
                ...
                document.querySelectorAll('.seba-checkbox').forEach(checkbox => {
                    checkbox.addEventListener('change', function () {
                        ...
                        let beddhaListDiv = document.getElementById('beddha_list');
                            ...
                            fetch(`/admin/get-beddha/${sebaId}`)
                                ...
                                .then(data => {
                                    let beddhaHtml = `<div class="beddha-group" id="beddha_group_${sebaId}">
                                        <strong>Beddha for ${this.nextElementSibling.innerText}:</strong>`;
                                    data.forEach(beddha => {
                                        beddhaHtml += `<div class="form-check">
                                    ...
                                    beddhaHtml += `</div>`;
                                    beddhaListDiv.innerHTML += beddhaHtml;
                                });
                        } else {
                            // Remove only the unchecked Seba's Beddha list
                            let removeDiv = document.getElementById(`beddha_group_${sebaId}`);
                            if (removeDiv) {
                                removeDiv.remove();
                            }
                        }
