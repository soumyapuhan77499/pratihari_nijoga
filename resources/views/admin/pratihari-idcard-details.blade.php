        ...
        <div class="col-12 mt-2">
            <div class="card shadow-lg">
                ...
                <ul class="nav nav-tabs flex-column flex-sm-row mt-2" role="tablist">
                    ...
                    <li class="nav-item col-12 col-sm-auto">
                        <a class="nav-link" id="id-card-tab" style="background-color: rgb(49, 49, 181);color: white"  data-toggle="tab" href="{{ route('admin.pratihariIdcard') }}" role="tab" aria-controls="id-card" aria-selected="false">
                ...
                <div class="card-body">
                    <form action="{{ route('admin.pratihari-idcard.store') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-3">
                                <button type="button" class="btn" id="add-id-btn" style="color: rgb(17, 39, 236);margin-top: 26px;width: 100%;text-align: center;background: linear-gradient(90deg, #ecf006, #d5e522);
">
                                    <i class="fas fa-plus"></i> Add ID
                                </button>
                            </div>

                            <div id="id-section" class="col-12">
...
<script>
    ...
    function addIDEntry() {
        ...
        newIdEntry.innerHTML = `
            ...
            <div class="col-md-3">
                <button type="button" class="btn remove-btn" style="color: white;margin-top: 26px; width: 100%; text-align: center;background-color: rgb(251, 51, 71);">
                    <i class="fas fa-minus"></i> Remove ID
                </button>
            </div>
        `;
    ...
    function updateAllDropdowns() {
        document.querySelectorAll(".id-type-select").forEach(select => {
            ...
            select.innerHTML = `<option value="" disabled>Select ID Type</option>` +
                availableOptions.map(opt => `<option value="${opt}" ${opt === selectedValue ? 'selected' : ''}>${opt}</option>`).join('');
        });
    }
